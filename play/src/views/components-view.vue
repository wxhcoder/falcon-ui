<template>
  <section class="page">
    <h2 class="page-title">Components</h2>
    <p class="page-desc">Real demos for the first batch: FButton and FInput.</p>

    <article class="card demo-card">
      <h3>FButton</h3>
      <div class="demo-row">
        <FButton type="default">Default</FButton>
        <FButton type="primary">Primary</FButton>
        <FButton type="success">Success</FButton>
        <FButton type="warning">Warning</FButton>
        <FButton type="danger">Danger</FButton>
      </div>
      <div class="demo-row">
        <FButton size="small">Small</FButton>
        <FButton size="medium">Medium</FButton>
        <FButton size="large">Large</FButton>
      </div>
      <div class="demo-row">
        <FButton :disabled="true">Disabled</FButton>
        <FButton :loading="true" type="primary">Loading</FButton>
        <FButton type="primary" @click="buttonClicks += 1">Click +1</FButton>
      </div>
      <p class="demo-result">Click count: {{ buttonClicks }}</p>
    </article>

    <article class="card demo-card">
      <h3>FInput</h3>
      <div class="demo-row">
        <FInput
          v-model="inputValue"
          placeholder="Type something..."
          :clearable="true"
          @input="inputEvents += 1"
          @change="changeEvents += 1" />
      </div>
      <div class="demo-row">
        <FInput v-model="disabledValue" placeholder="Disabled input" :disabled="true" />
      </div>
      <p class="demo-result">Current value: {{ inputValue || '(empty)' }}</p>
      <p class="demo-result">Input events: {{ inputEvents }}</p>
      <p class="demo-result">Change events: {{ changeEvents }}</p>
    </article>
  </section>
</template>

<script setup lang="ts">
import { ref } from 'vue'
import { FButton, FInput } from '@falcon-ui/falcon-ui'

const buttonClicks = ref(0)
const inputValue = ref('')
const disabledValue = ref('disabled text')
const inputEvents = ref(0)
const changeEvents = ref(0)
</script>

<style scoped>
.demo-card {
  margin-bottom: 12px;
}

.demo-row {
  display: flex;
  flex-wrap: wrap;
  gap: 10px;
  margin: 10px 0;
}

.demo-result {
  margin: 6px 0 0;
}
</style>
