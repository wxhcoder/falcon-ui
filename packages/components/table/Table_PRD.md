...
- `is-edit` 必须兼容 Vue 已响应式化的数据源，包括 `ref([])`、`reactive([])` 等主流写法。
- 直接字段绑定（如 `row.name`、`row.status`）与动态顶层 key 绑定（如 `row[column.key]`）都必须在真实输入组件链路中做到“输入即显示”，不允许等待 blur 后再补刷新。
- `cell-change` 事件上报与界面即时同步必须同时成立，不能只保证事件正确而牺牲输入过程中的视图回流。
- 深层嵌套路径（如 `row.profile.nickname`）的变更事件上报继续保留；若后续发现特定输入组件在嵌套路径场景仍有即时同步边界，再单独立项增强。

### 4.5 交叉高亮

- 目标：当用户点击某个数据单元格后，默认通过单元格焦点边框明确突出当前关注数据；开启
  `crossHighlight` 后，再扩展形成“当前行 + 当前列”的交叉高亮。
- 开关：
  - 新增 `crossHighlight` 属性，默认 `false`。
  - `highlight-current-row` 继续保持默认开启；交叉高亮的行/列背景直接复用当前行高亮变量。
  - `crossHighlight` 仅控制行/列表头扩展高亮，不控制当前单元格焦点边框是否建立。
- 激活源：
  - 仅限“表体数据单元格点击”。
  - 点击表头、表格内部空白区域、非数据单元格区域时，不建立新高亮。
  - 点击表格外任意区域时，清空当前单元格焦点与交叉高亮。
- 高亮范围：
  - 默认命中的当前单元格焦点边框。
  - 当前行的全部数据单元格。
  - 当前列对应的表头单元格与表体数据单元格。
...
- 状态模型：
  - 当前单元格焦点与交叉高亮共用 `activeCell = { rowKey, columnKey }`。
  - `rowKey` 来源固定为 `row[rowKeyField]`；若当前行取不到有效 `rowKey`，本次点击不建立焦点与交叉高亮。
...
- `crossHighlight=false` 时，点击普通数据单元格后，当前单元格立即出现焦点边框，但不扩展当前行、当前列与表头列高亮。
