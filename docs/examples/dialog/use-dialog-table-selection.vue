<template>
  <div class="demo-col">
    <FlButton type="primary" @click="openByUseDialog">useDialog 打开表格选择弹窗</FlButton>
    <div class="demo-result">最近确认行：{{ confirmedRowIds || '(none)' }}</div>
    <div class="demo-result">最近关闭动作：{{ latestRejectedAction || '(none)' }}</div>
  </div>
</template>
...
const renderSelectionTable = () =>
  h(
    ...
    () => [
      ...
      h(ElTableColumn, { label: '联系人', prop: 'contact', width: 140 })
    ]
  )
