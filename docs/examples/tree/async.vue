<template>
  <div class="demo-col">
    <div class="demo-toolbar">
      <FlButton size="small" @click="reset">重置异步节点</FlButton>
      <el-switch v-model="controlledLoaded" active-text="受控 loadedKeys" />
    </div>
    ...
    <div class="demo-result">最近加载：{{ latestLoad || '(none)' }}</div>
  </div>
</template>
...
const loadData = async (node: TreeNode) => {
  ...
  data.value = replaceChildren(data.value, node.key, [
    { key: `${String(node.key)}-member-a`, label: '成员 A', isLeaf: true },
    { key: `${String(node.key)}-member-b`, label: '成员 B', isLeaf: true }
  ])
}
