<template>
  <div class="demo-col">
    <FlInputSearch
      ...
      v-model:label="label"
      clearable
      placeholder="Enter acme and press Enter; useDialog renders a table and backfills selected rows"
      :fetch-api="fetchApi"
      :map-result="mapResult"
      @open-dialog="handleOpenDialog" />

    <div class="demo-result">openDialog count:{{ panelCount }}</div>
    <div class="demo-result">Latest reason:{{ panelReason || '(none)' }}</div>
    <div class="demo-result">
      Keyword / result count:{{ panelKeyword || '(empty)' }} / {{ panelSize }}
    </div>
    <div class="demo-result">
      Current backfill:{{ value ?? '(null)' }} / {{ label || '(empty)' }}
    </div>
    <div class="demo-result">useDialog Confirm count:{{ confirmCount }}</div>
    <div class="demo-result">
      useDialog Latest rejected action:{{ latestRejectedAction || '(none)' }}
    </div>
    <div class="demo-result">Latest confirmed rows:{{ latestConfirmedRows || '(none)' }}</div>
  </div>
</template>
...
const handleOpenDialog = (payload: {
  ...
  dialog
    .open<SearchItem[]>({
      title: 'Select customer (useDialog)',
